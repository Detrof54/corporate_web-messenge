...
// Получение информации о группе/канале (для формирования профиля)
  ...
  .query(async ({ ctx, input }) => {
    ...
    const sortedMembers = [...chat.members].sort((a, b) => {
      ...
      const roleCompare = rolePriority[a.role] - rolePriority[b.role];

      if (roleCompare !== 0) 
          return roleCompare;

      const nameA = `${a.user.firstname ?? ""} ${a.user.surname ?? ""}`.toLowerCase();
      const nameB = `${b.user.firstname ?? ""} ${b.user.surname ?? ""}`.toLowerCase();
...
// Обновление информации канала/группы
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    const currentMember = await ctx.db.chatMember.findFirst({
      where: {
        chatId: input.chatId,
    ...
    if (!currentMember || (currentMember.role !== ChatRole.OWNER && currentMember.role !== ChatRole.ADMIN)){
      throw new TRPCError({
        code: "FORBIDDEN",
      });
    }
    ...
    if (input.memberRoles?.length) {
      for (const memberRole of input.memberRoles) {
        const targetMember = await ctx.db.chatMember.findFirst({
          where: {
            chatId: input.chatId,
...
//Добавление пользователей в группу/канал
...
//Удаление пользователей из группы/канала
